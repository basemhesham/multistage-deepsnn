...
module stage3_top_tb;
    ...
    localparam int NUM_CHANNELS = 64;
    localparam int INPUT_SIDE   = 66;
    localparam int OUTPUT_SIDE  = 32;
    ...
    logic [(12*32*9*DATA_WIDTH)-1:0] sim_pixels;
    ...
    function automatic logic signed [DATA_WIDTH-1:0] reference_tree(
        input int group
    );
        longint signed channel_acc;
        longint signed tree_acc;
            ...
            for (int channel = 0; channel < 32; channel++) begin
                ...
                for (int tap = 0; tap < NUM_INPUTS; tap++) begin
                    pixel_value = $signed(sim_pixels[
                        ((((group * 32) + channel) * NUM_INPUTS) + tap) *
                         DATA_WIDTH +: DATA_WIDTH
                    ]);
                    weight_value =
                ...
                tree_acc += channel_result;
            end

            reference_tree = tree_acc[DATA_WIDTH-1:0];
        end
    endfunction
    ...
    function automatic logic signed [DATA_WIDTH-1:0] reference_conv(
        ...
        logic signed [DATA_WIDTH-1:0] first_half;
        logic signed [DATA_WIDTH-1:0] second_half;
        begin
            first_half  = reference_tree(window * 2);
            second_half = reference_tree((window * 2) + 1);
            reference_conv = first_half + second_half;
        end
    endfunction
    ...
    function automatic logic signed [DATA_WIDTH-1:0] reference_pool;
        ...
        begin
            ...
            for (int window = 0; window < 4; window++) begin
                ...
                bn_acc = ($signed(relu_result) * 18'sd432) +
                         (-18'sd306 <<< 9);
                bn_result = bn_acc >>> 9;
    ...
    initial begin
        ...
        $readmemb("bin_export/input/lif2_out_padded.txt", arr_in);
        ...
        $display("------------------------------------------------------------");
        $display("Checked Stage 3 output positions = %0d", checked_counter);
        $display("Total mismatches                 = %0d", failed_counter);

        if (failed_counter != 0)
            $fatal(1, "Stage 3 verification failed");

        $display(
            "PASS: deep_snn_top Stage 3 has no convolution, pool, or spike mismatches."
        );
        $finish;
    end
    ...
    initial begin
        force DUT.src_sel      = 2'b10;
        force DUT.conv3_filter = 7'd0;
        force DUT.mult_weight_param[0] = 18'sd432;
        force DUT.add_weight_param[0]  = -18'sd306;
    ...
    initial begin
        #(CLK_PERIOD * 5000);
        $fatal(1, "Timeout waiting for Stage 3 verification to finish");
    end
