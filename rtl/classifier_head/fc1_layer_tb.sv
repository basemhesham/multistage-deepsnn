...
module fc1_layer_tb ();
parameter DATA_WIDTH = 18 , NUM_OF_INPUTS = 128 , NUM_OF_OUTPUTS = 256 , CLK_PERIOD = 10;
...
initial
 ...
 for(int i =0; i<NUM_OF_INPUTS ; i++)
  ...
  $readmemb("gap.txt",arr_in);
  $display("  Loaded gap.txt : %0d values", NUM_OF_INPUTS);
  $readmemb("fc1.txt",expected_arr_out);
  $display("  Loaded fc1.txt : %0d values", NUM_OF_OUTPUTS);
 #CLK_PERIOD;
  ...
  for(int k=0 ; k<NUM_OF_OUTPUTS ; k++)
       begin
        if( expected_arr_out[k] == fc_out[k] )
         ...
         $display("  FAIL [%0d]: expected= %0b.%0b  actual= %0b.%0b", k, expected_arr_out[k][17:9],expected_arr_out[k][8:0], fc_out[k][17:9],fc_out[k][8:0]);                    
         end
        end
