...
module mapping_controller #(
    ...
    output logic [1:0]                        state_o        // 00=IDLE 01=LOAD 10=FETCH (for TB)
);
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            // ------------------------------------------------------------------
            // LOAD -> FETCH: reset conv position; rotate banks AFTER load completes
            // ------------------------------------------------------------------
            if (state == LOAD && next_state == FETCH) begin
                conv_row <= '0;
                conv_col <= '0;
            ...
            if (state == FETCH && next_state == IDLE) begin
                ...
                if (win_idx == NUM_H_WIN - 1) begin
                    ...
                    // Bank rotation happens at LOAD->FETCH after the partial load
                    ...
                    full_load       <= 1'b0;
                    // write_bank and fetch_order unchanged here — updated at LOAD->FETCH
                end
            end
